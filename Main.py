...
st.set_page_config(page_title="Institutional Pro v5.15", layout="wide")
st.title("🔴 XAUUSD Institutional Pro v5.15 (IST Display)")
...
# Shifted Timezone to IST
ist_tz = pytz.timezone('Asia/Kolkata')
now_ist = datetime.now(ist_tz)
hour_ist = now_ist.hour

# IST Session Logic Mapping
...
# Logic matching exact interface structure
sl, tp = None, None
if not is_session:
...
# Exact UI Match with IST label replacing NY
st.markdown(f"""
<div style="background-color: #1e293b; padding: 25px; border-radius: 15px; border-left: 12px solid {color}; color: #f8fafc;">
    ...
    <p><b>IST Time:</b> {now_ist.strftime('%H:%M:%S')} | <b>TF:</b> {tf} | <b>Feed:</b> Spot (XAUUSD=X)</p>
